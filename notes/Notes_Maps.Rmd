...
```{r}
ggplot() + 
  geom_sf(data = nc, col="black", fill="grey") + 
  theme_light()
...
```{r}
ggplot() + 
  geom_sf(data=nc[1:3,]) + #you may only want to plot a subset of the counties. In this case I subsetted the first 3 rows. You could also use filter. 
  aes(fill = BIR74) +
  geom_sf_text(data = nc[1:3, ], aes(label = NAME)) + # add name labels
...
- you should use a color scheme that is categorial, when your data is in categories and isn't ordered
- you should use a color scheme that is diverging, when want to put emphasis on two extremes and mid-range. For example, you might use a diversging palette from red to blue for political party affiliation in the US. 
...
You can break up sequential or diverging palettes into a fixed number of colours using `brewer.pal`. 
...
Also when using maps the `coord_fixed(ratio = 1)` is also super important so the aspect ratio is maintained (a country may appear super stretched or squished). This isn't too bad for the USA, but for a longer skinner country like Italy this can get pretty wonky. 
